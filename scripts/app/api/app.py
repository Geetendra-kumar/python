...
f = open(r"C:\Users\geete\Desktop\python\build\contracts\Bank.json", "r")
data = json.load(f)
abi = data["abi"]
...
contract.functions.deposit_funds().transact({"from": w3.eth.accounts[0], "value": 100})
...
tx = contract.functions.transfer_funds(w3.eth.accounts[2], 25).transact(
...
accs = {"1234": 0, "2345": 1, "3456": 2, "4567": 3, "5678": 4, "6789": 5}
...
def get_history():
    ...
    for num in blocks:
        ...
        if str(cd[0]) == "<Function transfer_funds(address,uint256)>":
            transactions.append(
                {
                    "id": str(tx.blockNumber),
                    "amount": str(cd[1]["_funds"]),
                    "sentTo": str(cd[1]["receiving_address"]),
                }
            )
...
def funds_transfer():
    ...
    tx = contract.functions.transfer_funds(rec_num, amount).transact(
        {"from": w3.eth.accounts[sender_num]}
    )
    return {"status": True}
